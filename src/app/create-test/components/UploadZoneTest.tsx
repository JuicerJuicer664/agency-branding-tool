...
function classifyImageQuality(img: HTMLImageElement, fileSizeBytes: number): QualityInfo {
  ...
  // Over-compression check: bytes per pixel < 0.5 suggests heavy JPEG compression
  const bytesPerPixel = fileSizeBytes / (w * h);
  if (bytesPerPixel < 0.5 && fileSizeBytes > 0) {
    warnings.push('Heavily compressed');
  }

  // Unusual aspect ratio check (outside 9:16 to 16:9 range)
  const ratio = w / h;
...
export default function UploadZoneTest({
  uploadedImage,
  uploadedFileName,
  onImageUpload,
  onImageRemove,
  ...
  const processFile = useCallback(
    (file: File) => {
      ...
      reader.onload = (e) => {
        ...
        if (result) {
          // Run quality check — non-blocking
          const img = new Image();
          img.onload = () => {
