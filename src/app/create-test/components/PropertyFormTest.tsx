...
interface PropertyFormTestProps {
  details: PropertyDetailsTest;
  errors: Partial<PropertyDetailsTest>;
  onChange: (updated: PropertyDetailsTest) => void;
  onListingTypeChange?: (type: string) => void;
  listingType?: string;
}

interface FieldConfig {
  key: keyof PropertyDetailsTest;
  label: string;
  placeholder: string;
  type: string;
  icon: string;
...
export default function PropertyFormTest({
  ...
  onListingTypeChange,
  listingType = 'Listed',
}: PropertyFormTestProps) {
  const handleChange = (key: keyof PropertyDetailsTest, value: string) => {
        ...
        {fields.map((field) => (
          <div key={field.key} className={field.colSpan || 'col-span-1'}>
            ...
            <input
              ...
              aria-describedby={errors[field.key] ? `test-error-${field.key}` : undefined}
            />
            {errors[field.key] && (
              <p
                id={`test-error-${field.key}`}
